async function handler(m, {usedPrefix, command}) {
  ...
  switch (command) {
    case 'next':
    case 'leave': {
      const room = Object.👑 𝗞𝗜𝗟𝗟𝗭𝗡 👑es(this.anonymous).find((room) => room.check(m.sender));
    ...
    case 'start': {
      if (Object.👑 𝗞𝗜𝗟𝗟𝗭𝗡 👑es(this.anonymous).find((room) => room.check(m.sender))) return this.sendMessage(m.chat, {text: `*[❗𝐈𝐍𝐅𝐎❗] 𝚃𝙾𝙳𝙰𝚅𝙸𝙰 𝙴𝚂𝚃𝙰𝚂 𝙴𝙽 𝚄𝙽 𝙲𝙷𝙰𝚃 𝙰𝙽𝙾𝙽𝙸𝙼𝙾 𝙾 𝙴𝚂𝙿𝙴𝚁𝙰𝙽𝙳𝙾 𝙰 𝚀𝚄𝙴 𝙾𝚃𝚁𝙾 𝚄𝚂𝚄𝙰𝚁𝙸𝙾 𝚂𝙴 𝚄𝙽𝙰 𝙿𝙰𝚁𝙰 𝙸𝙽𝙸𝙲𝙸𝙰𝚁*\n\n*¿𝚀𝚄𝙸𝙴𝚁𝙴𝚂 𝚂𝙰𝙻𝙸𝚁 𝙳𝙴𝙻 𝙲𝙷𝙰𝚃 𝙰𝙽𝙾𝙽𝙸𝙼𝙾?*\n𝙴𝚂𝙲𝚁𝙸𝙱𝙴 ${usedPrefix}leave`}, {quoted: m});
      ...
      const room = Object.👑 𝗞𝗜𝗟𝗟𝗭𝗡 👑es(this.anonymous).find((room) => room.state === 'WAITING' && !room.check(m.sender));
      if (room) {
